...
RUN cd build_jaxlib && python build/build.py --enable_cuda --dev_install --bazel_options=--override_repository=org_tensorflow=$(pwd)/../third_party/tensorflow-alpa && cd dist && pip install -e .
